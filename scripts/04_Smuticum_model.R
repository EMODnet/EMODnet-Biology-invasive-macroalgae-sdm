...
# Load, filter, downsample and save Sargassum muticum occurrences
load("./occurrences.Rda")
Sargassum.raw <- occurrences %>% filter(name=="Sargassum")
Sargassum <- data.frame(ensemble.spatialThin(Sargassum.raw[,c("longitude","latitude")], thin.km = 9.2,
                                             runs = 50, silent = FALSE, verbose = FALSE,
                                             return.notRetained = FALSE))

presences <- kuenm_occsplit(Sargassum, train.proportion=0.75, save=FALSE)
presences[[1]]$species <- "Sargassum"
presences[[1]]<- presences[[1]][,c(3,1,2)]
presences[[2]]$species <- "Sargassum"
presences[[2]]<- presences[[2]][,c(3,1,2)]
presences[[3]]$species <- "Sargassum"
presences[[3]]<- presences[[3]][,c(3,1,2)]

dir.create("./analysis/kuenm_Sargassum", recursive = TRUE)
write.csv(presences[[1]],
          file="./analysis/kuenm_Sargassum/Sargassum_joint.csv", row.names=FALSE, quote=F)
write.csv(presences[[2]],
          file="./analysis/kuenm_Sargassum/Sargassum_train.csv", row.names=FALSE, quote=F)
write.csv(presences[[3]],
          file="./analysis/kuenm_Sargassum/Sargassum_test.csv", row.names=FALSE, quote=F)

# Create sets of variables with a minimum of 5 variables by group
vs <- kuenm_varcomb(var.dir = './derived_data/env/hist',
                    out.dir = './analysis/kuenm_Sargassum/M_variables', min.number = 5,
...
occ_joint <- "./analysis/kuenm_Sargassum/Sargassum_joint.csv"
occ_tra <- "./analysis/kuenm_Sargassum/Sargassum_train.csv"
M_var_dir <- "./analysis/kuenm_Sargassum/M_variables"
batch_cal <- "./analysis/kuenm_Sargassum/Candidate_models"
out_dir <- "./analysis/kuenm_Sargassum/Candidate_Models"
...
# Models evaluation
#help(kuenm_ceval)
occ_test <- "./analysis/kuenm_Sargassum/Sargassum_test.csv"
out_eval <- "./analysis/kuenm_Sargassum/Calibration_results"
threshold <- 10
rand_percent <- 50
iterations <- 10
kept <- TRUE
...
g_vs <- kuenm_varcomb(var.dir = './derived_data/env/RCP85',
                      out.dir = './analysis/kuenm_Sargassum/G_variables', min.number = 5,
                      in.format = 'ascii', out.format = 'ascii')

# Move to a subfolder to met te required format for the function kuen_mod
g_dir <- list.files("./analysis/kuenm_Sargassum/G_variables", full.names = TRUE)
...
mod_dir <- "./analysis/kuenm_Sargassum/Final_Models"
rep_n <- 10
rep_type <- "Bootstrap"
jackknife <- TRUE
out_format <- "logistic"
project <- TRUE
G_var_dir <- "./analysis/kuenm_Sargassum/G_variables"
ext_type <- "all"
write_mess <- TRUE
write_clamp <- TRUE
wait1 <- FALSE
...
# help(kuenm_feval)
occ_ind <- "./analysis/kuenm_Sargassum/Sargassum_test.csv"
replicates <- TRUE
out_feval <- "./analysis/kuenm_Sargassum/Final_Models_evaluation"
